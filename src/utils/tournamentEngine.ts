...
export function createInitialTournament(teamNames: string[] = INITIAL_TEAMS_DEFAULT, tournamentName: string = DEFAULT_SETTINGS.name): TournamentState {
  ...
  // --- ROUND 5: BRONZE (3rd place match) ---
  matches['BRONZE'] = {
    id: 'BRONZE',
    roundId: 'BRONZE',
    matchNumber: 1,
...
/**
 * Helper to sync losers of 1/4 (Semi-Final) matches into 3rd Place match
 */
export function syncSemiFinalLosers(matches: Record<string, Match>) {
  if (!matches['BRONZE']) {
    matches['BRONZE'] = {
      id: 'BRONZE',
      roundId: 'BRONZE',
  ...
  const bronze = matches['BRONZE'];
  const r4m1 = matches['R4_M1'];
  const r4m2 = matches['R4_M2'];

  // Slot 1 from R4_M1 loser
  if (r4m1 && r4m1.status === 'completed' && r4m1.winnerId) {
    const loser1 = r4m1.winnerId === r4m1.team1?.id ? r4m1.team2 : r4m1.team1;
    if (bronze.team1?.id !== loser1?.id) {
      bronze.team1 = loser1 ? { ...loser1 } : null;
      bronze.score1 = null;
      bronze.score2 = null;
  ...
  // Slot 2 from R4_M2 loser
  if (r4m2 && r4m2.status === 'completed' && r4m2.winnerId) {
    const loser2 = r4m2.winnerId === r4m2.team1?.id ? r4m2.team2 : r4m2.team1;
    if (bronze.team2?.id !== loser2?.id) {
      bronze.team2 = loser2 ? { ...loser2 } : null;
      bronze.score1 = null;
      bronze.score2 = null;
      bronze.winnerId = null;
      bronze.status = 'scheduled';
...
export function updateMatchScore(
  ...
  // Sync losers of 1/4 semi-finals into 3rd Place match
  syncSemiFinalLosers(newMatches);
