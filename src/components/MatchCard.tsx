...
export const MatchCard: React.FC<MatchCardProps> = ({
  ...
  const roundTitles: Record<string, string> = {
    ...
    R4: '1/2',
    FINAL: 'FINAL (1-O\'RIN)',
    BRONZE: '3-O\'RIN UCHUN',
  };

  const isFinal = match.roundId === 'FINAL';
  const isBronze = match.roundId === 'BRONZE';

  return (
    <div
      className={`relative ${
        isFinal
          ? 'w-56 bg-gradient-to-br from-white to-amber-50/40 border-2 border-yellow-400 shadow-xl ring-2 ring-yellow-100/80 p-3'
          : isBronze
          ? 'w-52 bg-gradient-to-br from-white to-amber-50/20 border-2 border-amber-300 shadow-md p-2.5'
          : 'w-44 bg-white shadow-md border-l-4 border-gray-300 p-2 border-y border-r border-gray-200'
      } rounded-lg text-[11px] select-none transition-all duration-150 ${
        isDrawError
    ...
    >
      ...
      <div className="flex items-center justify-between mb-1">
        <span className={`text-[9px] font-extrabold uppercase tracking-wider ${isFinal ? 'text-amber-700' : isBronze ? 'text-amber-800' : 'text-gray-400'}`}>
          {isFinal ? '🏆 Final' : isBronze ? '🥉 3-O\'rin' : `Match #${match.matchNumber} - ${roundTitles[match.roundId]}`}
        </span>
